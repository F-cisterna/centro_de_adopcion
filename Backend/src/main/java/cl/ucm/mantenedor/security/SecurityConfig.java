    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        .requestMatchers(HttpMethod.POST, "/api/solicitudes", "/api/solicitudes/**").hasAnyAuthority("ROLE_USER", "ROLE_ADMIN", "USER", "ADMIN")
                        .requestMatchers(HttpMethod.POST, "/api/refugios", "/api/refugios/**", "/api/animales", "/api/animales/**").hasAnyAuthority("ROLE_ADMIN", "ADMIN")
                        .requestMatchers(HttpMethod.PUT, "/api/solicitudes", "/api/solicitudes/**").hasAnyAuthority("ROLE_USER", "ROLE_ADMIN", "USER", "ADMIN")
                        ...
                        .requestMatchers("/api/adoptantes-user/**").hasAnyAuthority("ROLE_USER", "ROLE_ADMIN", "USER", "ADMIN")

                        .anyRequest().authenticated()
                )
                .exceptionHandling(exception -> exception
                        .authenticationEntryPoint((request, response, authException) -> {
                            response.sendError(jakarta.servlet.http.HttpServletResponse.SC_UNAUTHORIZED, "No autorizado (Token faltante o inválido)");
                        })
                )
